...
import { Download, RefreshCw, CheckCircle2, AlertCircle, Clock, DollarSign, Users, FileText, Settings as SettingsIcon, ExternalLink } from "lucide-react";
...
export default function AdminPayoutDashboard() {
  ...
  const { data: pendingPayouts, isLoading: loadingPending, refetch: refetchPending } = useQuery({
        ...
        <Card className="shadow-2xl border-4" style={{ borderColor: '#00BFFF' }}>
          <CardHeader>
            ...
            <p className="text-base font-semibold mt-2" style={{ color: '#4A90E2' }}>
              Manage streamer payouts, onboard contractors, and export payout CSV files for Mellow.io.
            </p>
          </CardHeader>
        </Card>

        <Tabs defaultValue="pending" className="w-full">
          <TabsList className="grid grid-cols-5 gap-2 p-1.5 rounded-xl shadow-xl w-full" style={{ backgroundColor: '#E0F4FF' }}>
              ...
              <CardContent className="p-6">
                ...
                <div className="space-y-3">
                  {pendingPayouts.map((payout) => (
                    <div key={payout.broadcaster.id} className="flex items-center gap-4 p-4 rounded-lg border-2" style={{ borderColor: '#E0F4FF' }}>
                      <Checkbox
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600">{payout.broadcaster.country}</p>
                        {payout.contractorAccount?.contractor_id && (
                          <p className="text-xs text-gray-500">ID: {payout.contractorAccount.contractor_id}</p>
                        )}
                      </div>
              ...
              <CardContent className="p-6">
                <div className="space-y-3">
                  {completedPayouts.map((export_) => (
                    <div key={export_.id} className="p-4 rounded-lg border-2" style={{ borderColor: '#E0F4FF' }}>
          ...
          <TabsContent value="streamers" className="space-y-4 mt-6">
            <Card className="shadow-xl border-2" style={{ borderColor: '#87CEEB' }}>
              <CardContent className="p-6">
                <div className="mb-4">
                  <Button
                ...
                <div className="space-y-3">
                  {allStreamers.map((streamer) => (
                    <div key={streamer.broadcaster.id} className="p-4 rounded-lg border-2" style={{ borderColor: '#E0F4FF' }}>
                      <div className="flex items-center justify-between">
                        <div>
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    />
                    <span className="text-sm">{settings?.notify_streamer_payouts ? 'Enabled' : 'Disabled'}</span>
                  </div>
                </div>
